...
const LAMPORTS_PER_SOL = 1000000000; // 1 SOL = 10^9 lamports
...
try {
    ...
    payoutKeypair = web3_js_1.Keypair.fromSecretKey(bs58_1.default.decode(PAYOUT_WALLET_PRIVATE_KEY));
...
router.post("/payout", middleware_1.workerMiddleware, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    //@ts-ignore
    const workerId = req.userId; // Correctly typed now, assuming workerMiddleware sets userId
    if (!workerId) {
    ...
    try {
        ...
        worker = yield prismaClient.worker.findFirst({
    ...
    try {
        yield prismaClient.$transaction((tx) => __awaiter(void 0, void 0, void 0, function* () {
            ...
            const currentWorker = yield tx.worker.findUnique({
                where: { id: Number(workerId) }, // Use workerId directly as string
    ...
    try {
        ...
        web3_js_1.ComputeBudgetProgram.setComputeUnitPrice({ microLamports: PRIORITY_RATE }), web3_js_1.SystemProgram.transfer({
            fromPubkey: payoutKeypair.publicKey, // Funds come from the backend's payout wallet
            toPubkey: new web3_js_1.PublicKey(worker.address), // Funds go to the worker's address
        ...
        signature = yield (0, web3_js_1.sendAndConfirmTransaction)(connection, transaction, [payoutKeypair], // Sign with the backend's payout keypair
        {
        ...
        yield prismaClient.payouts.create({
            data: {
